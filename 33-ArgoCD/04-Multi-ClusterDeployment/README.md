...
The `cluster.yaml` for the clusters should look something like this (so that the API server is reachable from outside the cluster):
   ...
   networking:
     apiServerAddress: "172.31.0.101" # Where this should be the external IP address of the virtual machine
     apiServerPort: 6443
   nodes:
   - role: control-plane
...
In the `argocd` directory, we create a new file called `onepageserver-primary.yaml` with the following contents:
   ...
   spec:
     ...
     source:
       ...
       targetRevision: main
     destination:
       server: 'https://172.31.0.101:6443'
       namespace: default
     syncPolicy:
       automated:
         selfHeal: true
...
Go to each server and view the page including the load balancer: 172.31.0.101, 172.31.0.102, 192.168.2.37.
